...
+ function($) {
  ...
  $.fn.cargoPicker = function(params) {
    params = $.extend({}, defaults, params);
    return this.each(function() {
      var self = this;
  ...
  defaults = $.fn.cargoPicker.prototype.defaults = {
    showDistrict: true //是否显示地区选择
  };
...
+function($){
	  $.ajax({
	      type: "get",          
	      url: "/wss/insure/get_packs_list/",     
	      async: false,　　　　　　　　//是否为异步请求，ture为异步请求，false为同步请求
	      dataType: "json",
	      success: function(data) {    //当请求之后调用。
	    	  d = jQuery.parseJSON(data['data']['packs_list']);
	          $.rawPacksData =d;
	      },
      error:function(){
        alert("网络超时，用户操作失败");
...
+ function($) {
  ...
  defaults = $.fn.packPicker.prototype.defaults = {
    showDistrict: true //是否显示地区选择
  };
